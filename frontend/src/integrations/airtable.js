...
export const AirtableIntegration = ({ user, org, integrationParams, setIntegrationParams }) => {
    ...
    const handleConnectClick = async () => {
        try {
            ...
            const response = await axios.post(`http://localhost:8080/integrations/airtable/authorize`, formData);
    ...
    const handleWindowClosed = async () => {
        try {
            ...
            formData.append('user_id', user);
            formData.append('org_id', org);
            const response = await axios.post(`http://localhost:8080/integrations/airtable/credentials`, formData);
