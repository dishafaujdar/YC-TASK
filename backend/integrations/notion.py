...
CLIENT_ID = '1a6d872b-594c-8023-9cb5-00377db57ee8'
CLIENT_SECRET = 'Xsecret_J52UfJGHxb3FhrFsnHvodvyMCpTS9yw7CTlXv7qIkpEXX'

# CLIENT_ID = 'XXX'
# CLIENT_SECRET = 'XXX'

encoded_client_id_secret = base64.b64encode(f'{CLIENT_ID}:{CLIENT_SECRET}'.encode()).decode()   #hased the id & secret

REDIRECT_URI = 'http://localhost:8080/integrations/notion/oauth2callback'   # Redirect URI (where Notion will send the response)


authorization_url = f'https://api.notion.com/v1/oauth/authorize?client_id={CLIENT_ID}&response_type=code&owner=user&redirect_uri=http%3A%2F%2Flocalhost%3A8080%2Fintegrations%2Fnotion%2Foauth2callback'

# THE PRIMARY STEP OF SENDING REQUEST
...
#THE REQ'S RESPONSE WILL BE HANDEL BY THIS {ERROR || SUCCESS} 
async def oauth2callback_notion(request: Request):
    ...
    await add_key_value_redis(f'notion_credentials:{org_id}:{user_id}', json.dumps(response.json()), expire=600) # Once receive the access token, it store it in Redis. Which allows backend to make API requests to Notion on my behalf
...
# THE ABOVE API WILL RETURN THE TOKEN WHICH CONTAIN INFO. HERE THIS WILL BE SET IN REDIS AS {notion_credentials}
...
# this might be useful in searching for the values in the Oauth token
...
# TO ACCESS THE INGO/PAGES IN THE NOTION USER WILL NEED SOME SPECIFIC METADATA'S WHICH WILL BE PROVIED BY THIS
...
# AFTER SUCCESSFULL AND RIGHT PARSING OF THE METADATA, THIS WILL GRANT THE ACCES TO THE NOTION PAGE {send post req to the notion}
